    ...
    results.forEach(result => {
        ...
        <div class="readObject_Container target card mb-4">
            <div class="readObject_header card-header">
                <div class="row">
                    <form action="../controladores/edits/UpdateProveedores.php" method="post" class="form_edit col-auto d-flex align-items-center">
                        <input type="hidden" name="id" value="${result.id_proveedor}">
                        <button class="button btn btn-primary"> Editar</button>
                    </form>
                    <form action="../controladores/deletes/DeleteProveedor.php" method="post" class="form_edit col-auto me-auto d-flex align-items-center">
                        <input type="hidden" name="id" value="${result.id_proveedor}">
                    ...
                    <div class="accordion col-auto">
                        <div class="row accordion-header">
                            ...
                            data-bs-toggle="collapse" data-bs-target="#Accordion${result.id_proveedor}">
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div class="card-body row">
                <div class="data_container col-lg-12">
                    <h2>${highlightKeyword(keyword, result.nombre_empresa)}</h2>
                    <div class="d-flex justify-content-end">
                        <h4>${highlightKeyword(keyword, result.direccion)}</h4>
                    </div>
                </div>
            </div>
                ...
                <div class="readObject_Container target card mb-4 me-2 col-2 p-0">
                    <div class="readObject_header card-header">
                        <div class="row justify-content-between">
                            <form action="../controladores/edits/UpdateProveedores.php" method="post" class="form_edit col-auto d-flex align-items-center pe-0">
                                <input class="" type="hidden" name="id" value="${result.id_proveedor}">
                                <button class="button btn btn-primary btn-sm px-1"> Editar</button>
                            </form>
                            <form action="../controladores/deletes/DeleteProveedor.php" method="post" class="form_edit col-auto d-flex align-items-center pe-0">
                                <input class="" type="hidden" name="id" value="${result.id_proveedor}">
                    ...
                    <div class="principal_data card-body justify-content-center" style="flex: none;">
                        <h4 class="pe-0">${result.nombre_empresa}</h4>
                        <h6 class="pe-0">${result.direccion}</h6>
                    </div>
                    <div class="data_container hidde card-footer collapse" id="Accordion${result.id_proveedor}">
                        <div class="row">
                            <div class="product_info col-lg-12">
                                <h5>Datos</h5>
                                <h6 class="text-truncate"><small>Contacto: ${result.persona_contacto} g</small></h6>
                                <h6><small>Teléfono: ${result.num_telefono}</small></h6>
                                <h6 class="text-truncate"><small>Correo: ${result.email_proveedor}</small></h6>
                            </div>
                        </div>
                    </div>
                </div>
...
// AJAX CALLS
 function search(){
    ...
    $.ajax({
        url: '../controladores/gets/SearchProveedor.php',
        type: 'POST',
